...
begin = time.clock()
with open('./objs_data/10_10/objs_high2613.pkl', 'rb') as objs_file:
    init_test_x_and_y, init_test_x_and_y_high_low, train_x_set, train_y_set, model_save_path = pickle.load(objs_file)
...
"""
"""
仔细考虑清楚下面哪些情况该用high_low哪些情况该用正常的。
"""
...
for iteration_order in range(iteration):
    ...
    if iteration_order == 0:
        ...
        test_x_and_y = init_test_x_and_y_high_low
    test_x_and_y = test_x_and_y.to(device)
    test_x_and_y_high_low = test_x_and_y_high_low.to(device)
    model_SGD.eval()
    a_equal_d_SGD = model_SGD(test_x_and_y)  # test_x， a_equal_d， test_y这些对应的公式还没弄上去，还有数据的维度，类型还没弄好，先搭好一个框架
    a_equal_d_SGD_high_low = model_SGD(test_x_and_y_high_low)  # test_x， a_equal_d， test_y这些对应的公式还没弄上去，还有数据的维度，类型还没弄好，先搭好一个框架
    a_equal_d_SGD = a_equal_d_SGD.cpu()
    a_equal_d_SGD_high_low = a_equal_d_SGD_high_low.cpu()
    print("a_equal_d_SGD:", a_equal_d_SGD)
    print("a_equal_d_SGD_high_low:", a_equal_d_SGD_high_low)
    a_equal_d_SGD_aver = a_equal_d_SGD.sum() / (BATCH_SIZE * buyer_number * seller_number)
    a_equal_d_SGD_high_low_aver = a_equal_d_SGD_high_low.sum() / (BATCH_SIZE * buyer_number * seller_number)
    SumWriter.add_scalar("test_a_equal_d_SGD_aver", a_equal_d_SGD_aver.item(),
                         global_step=iteration_order)
    SumWriter.add_scalar("test_a_equal_d_SGD_high_low_aver", a_equal_d_SGD_high_low_aver.item(),
                         global_step=iteration_order)
    sw_SGD, sw_sum_SGD = sw_iter_BAP.sw_iter_BAP(a_equal_d=a_equal_d_SGD_high_low, label="SGD", iteration=iteration_order)
    ...
    test_x_high_low, test_y_high_low = derivation_x_y.derivation_x_y(a_equal_d=a_equal_d_SGD_high_low)
    test_x_SGD_aver = test_x.sum() / (BATCH_SIZE * buyer_number * seller_number)
    test_x_SGD_high_low_aver = test_x_high_low.sum() / (BATCH_SIZE * buyer_number * seller_number)
    SumWriter.add_scalar("test_x_SGD_aver", test_x_SGD_aver.item(),
                         global_step=iteration_order)
    SumWriter.add_scalar("test_x_SGD_high_low_aver", test_x_SGD_high_low_aver.item(),
                         global_step=iteration_order)
    test_y_SGD_aver = test_y.sum() / (BATCH_SIZE * buyer_number * seller_number)
    test_y_SGD_high_low_aver = test_y_high_low.sum() / (BATCH_SIZE * buyer_number * seller_number)
    SumWriter.add_scalar("test_y_SGD_aver", test_y_SGD_aver.item(),
                         global_step=iteration_order)
    SumWriter.add_scalar("test_y_SGD_high_low_aver", test_y_SGD_high_low_aver.item(),
                         global_step=iteration_order)
    bm_sum_SGD, sm_sum_SGD, pr_gap_sum_SGD, p_SGD, r_SGD = bm_sm_Iter_BAP.bm_sm_Iter_BAP(a_equal_d=a_equal_d_SGD_high_low,
                                                                                         input_x=test_x,
                                                                                         input_y=test_y,
                                                                                         label="SGD",
    ...
    test_x_and_y_high_low = combination_x_y.combination_x_y_max(test_x_high_low, test_y_high_low)
...
for iteration_order in range(iteration):
    ...
    if iteration_order == 0:
        test_x_and_y = init_test_x_and_y
        test_x_and_y_high_low = init_test_x_and_y_high_low
    test_x_and_y = test_x_and_y.to(device)
    test_x_and_y_high_low = test_x_and_y_high_low.to(device)
    model_Momentum.eval()
    a_equal_d_Momentum = model_Momentum(test_x_and_y)  # test_x， a_equal_d， test_y这些对应的公式还没弄上去，还有数据的维度，类型还没弄好，先搭好一个框架
    a_equal_d_Momentum_high_low = model_Momentum(test_x_and_y_high_low)  # test_x， a_equal_d， test_y这些对应的公式还没弄上去，还有数据的维度，类型还没弄好，先搭好一个框架
    a_equal_d_Momentum = a_equal_d_Momentum.cpu()
    a_equal_d_Momentum_high_low = a_equal_d_Momentum_high_low.cpu()
    print("a_equal_d_Momentum:", a_equal_d_Momentum)
    print("a_equal_d_Momentum_high_low:", a_equal_d_Momentum_high_low)
    a_equal_d_Momentum_aver = a_equal_d_Momentum.sum() / (BATCH_SIZE * buyer_number * seller_number)
    a_equal_d_Momentum_high_low_aver = a_equal_d_Momentum_high_low.sum() / (BATCH_SIZE * buyer_number * seller_number)
    SumWriter.add_scalar("test_a_equal_d_Momentum_aver", a_equal_d_Momentum_aver.item(),
                         global_step=iteration_order)
    SumWriter.add_scalar("test_a_equal_d_Momentum_high_low_aver", a_equal_d_Momentum_high_low_aver.item(),
                         global_step=iteration_order)
    sw_Momentum, sw_sum_Momentum = sw_iter_BAP.sw_iter_BAP(a_equal_d=a_equal_d_Momentum_high_low, label="Momentum", iteration=iteration_order)
    ...
    test_x_high_low, test_y_high_low = derivation_x_y.derivation_x_y(a_equal_d=a_equal_d_Momentum_high_low)
    test_x_Momentum_aver = test_x.sum() / (BATCH_SIZE * buyer_number * seller_number)
    test_x_Momentum_high_low_aver = test_x_high_low.sum() / (BATCH_SIZE * buyer_number * seller_number)
    SumWriter.add_scalar("test_x_Momentum_aver", test_x_Momentum_aver.item(),
                         global_step=iteration_order)
    SumWriter.add_scalar("test_x_Momentum_high_low_aver", test_x_Momentum_high_low_aver.item(),
                         global_step=iteration_order)
    test_y_Momentum_aver = test_y.sum() / (BATCH_SIZE * buyer_number * seller_number)
    test_y_Momentum_high_low_aver = test_y_high_low.sum() / (BATCH_SIZE * buyer_number * seller_number)
    SumWriter.add_scalar("test_y_Momentum_aver", test_y_Momentum_aver.item(),
                         global_step=iteration_order)
    SumWriter.add_scalar("test_y_Momentum_high_low_aver", test_y_Momentum_high_low_aver.item(),
                         global_step=iteration_order)
    bm_sum_Momentum, sm_sum_Momentum, pr_gap_sum_Momentum, p_Momentum, r_Momentum = bm_sm_Iter_BAP.bm_sm_Iter_BAP(a_equal_d=a_equal_d_Momentum_high_low,
                                                                                                                  input_x=test_x,
                                                                                                                  input_y=test_y,
                                                                                                                  label="Momentum",
                                                                                                                  iteration=iteration_order)
...
for iteration_order in range(iteration):
    ...
    a_equal_d_RMSprop_high_low = model_RMSprop(test_x_and_y_high_low)
    a_equal_d_RMSprop = a_equal_d_RMSprop.cpu()
    a_equal_d_RMSprop_high_low = a_equal_d_RMSprop_high_low.cpu()
    print("a_equal_d_RMSprop:", a_equal_d_RMSprop)
    print("a_equal_d_RMSprop_high_low:", a_equal_d_RMSprop_high_low)
    a_equal_d_RMSprop_aver = a_equal_d_RMSprop.sum() / (BATCH_SIZE * buyer_number * seller_number)
    a_equal_d_RMSprop_high_low_aver = a_equal_d_RMSprop_high_low.sum() / (BATCH_SIZE * buyer_number * seller_number)
    SumWriter.add_scalar("test_a_equal_d_RMSprop_aver", a_equal_d_RMSprop_aver.item(),
                         global_step=iteration_order)
    SumWriter.add_scalar("test_a_equal_d_RMSprop_high_low_aver", a_equal_d_RMSprop_high_low_aver.item(),
                         global_step=iteration_order)
    sw_RMSprop, sw_sum_RMSprop = sw_iter_BAP.sw_iter_BAP(a_equal_d=a_equal_d_RMSprop_high_low, label="RMSprop", iteration=iteration_order)
    ...
    test_x_high_low, test_y_high_low = derivation_x_y.derivation_x_y(a_equal_d=a_equal_d_RMSprop_high_low)
    test_x_RMSprop_aver = test_x.sum() / (BATCH_SIZE * buyer_number * seller_number)
    test_x_RMSprop_high_low_aver = test_x_high_low.sum() / (BATCH_SIZE * buyer_number * seller_number)
    SumWriter.add_scalar("test_x_RMSprop_aver", test_x_RMSprop_aver.item(),
                         global_step=iteration_order)
    SumWriter.add_scalar("test_x_RMSprop_high_low_aver", test_x_RMSprop_high_low_aver.item(),
                         global_step=iteration_order)
    test_y_RMSprop_aver = test_y.sum() / (BATCH_SIZE * buyer_number * seller_number)
    test_y_RMSprop_high_low_aver = test_y_high_low.sum() / (BATCH_SIZE * buyer_number * seller_number)
    SumWriter.add_scalar("test_y_RMSprop_aver", test_y_RMSprop_aver.item(),
                         global_step=iteration_order)
    SumWriter.add_scalar("test_y_RMSprop_high_low_aver", test_y_RMSprop_high_low_aver.item(),
                         global_step=iteration_order)
    bm_sum_RMSprop, sm_sum_RMSprop, pr_gap_sum_RMSprop, p_RMSprop, r_RMSprop = bm_sm_Iter_BAP.bm_sm_Iter_BAP(a_equal_d=a_equal_d_RMSprop_high_low,
                                                                                                             input_x=test_x,
                                                                                                             input_y=test_y,
                                                                                                             label="RMSprop",
                                                                                                             iteration=iteration_order)
...
for iteration_order in range(iteration):
    ...
    print("Adam mean of initial x", test_x_and_y[0][0].mean())
    print("Adam mean of initial x high low", test_x_and_y_high_low[0][0].mean())
    kkkkkkk = input()
    ...
    a_equal_d_Adam_high_low = model_Adam(test_x_and_y_high_low)
    a_equal_d_Adam = a_equal_d_Adam.cpu()
    a_equal_d_Adam_high_low = a_equal_d_Adam_high_low.cpu()
    print("a_equal_d_Adam:", a_equal_d_Adam)
    print("a_equal_d_Adam_high_low:", a_equal_d_Adam_high_low)
    a_equal_d_Adam_aver = a_equal_d_Adam.sum() / (BATCH_SIZE * buyer_number * seller_number)
    a_equal_d_Adam_high_low_aver = a_equal_d_Adam_high_low.sum() / (BATCH_SIZE * buyer_number * seller_number)
    SumWriter.add_scalar("test_a_equal_d_Adam_aver", a_equal_d_Adam_aver.item(),
                         global_step=iteration_order)
    SumWriter.add_scalar("test_a_equal_d_Adam_high_low_aver", a_equal_d_Adam_high_low_aver.item(),
                         global_step=iteration_order)
    sw_Adam, sw_sum_Adam = sw_iter_BAP.sw_iter_BAP(a_equal_d=a_equal_d_Adam_high_low, label="Adam", iteration=iteration_order)
    ...
    test_x_high_low, test_y_high_low = derivation_x_y.derivation_x_y(a_equal_d=a_equal_d_Adam_high_low)
    test_x_Adam_aver = test_x.sum() / (BATCH_SIZE * buyer_number * seller_number)
    test_x_Adam_high_low_aver = test_x_high_low.sum() / (BATCH_SIZE * buyer_number * seller_number)
    SumWriter.add_scalar("test_x_Adam_aver", test_x_Adam_aver.item(),
                         global_step=iteration_order)
    SumWriter.add_scalar("test_x_Adam_high_low_aver", test_x_Adam_high_low_aver.item(),
                         global_step=iteration_order)
    test_y_Adam_aver = test_y.sum() / (BATCH_SIZE * buyer_number * seller_number)
    test_y_Adam_high_low_aver = test_y_high_low.sum() / (BATCH_SIZE * buyer_number * seller_number)
    SumWriter.add_scalar("test_y_Adam_aver", test_y_Adam_aver.item(),
                         global_step=iteration_order)
    SumWriter.add_scalar("test_y_Adam_high_low_aver", test_y_Adam_high_low_aver.item(),
                         global_step=iteration_order)
    bm_sum_Adam, sm_sum_Adam, pr_gap_sum_Adam, p_Adam, r_Adam = bm_sm_Iter_BAP.bm_sm_Iter_BAP(a_equal_d=a_equal_d_Adam_high_low,
                                                                                              input_x=test_x,
                                                                                              input_y=test_y,
                                                                                              label="Adam",
                                                                                              iteration=iteration_order)
