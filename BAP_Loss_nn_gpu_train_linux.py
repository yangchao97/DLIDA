...
test_x_set = []
test_y_set = []
train_x_set, train_y_set, test_x_set, test_y_set = creation_data_x_y.creation_data_x_y()  # 买卖双方分别生成1000组报价数据，每组报价数据包含数据个数10*10，train_set：all_data_set = 0.8
...
test_data = TestData()     # test_data却别开来，一个是test_data，另外一个是test_data_high,后续涉及test_data的都需要修改，而且特别注意的是什么时候用test_data，什么时候用test_data_high。
...
with open('./objs_data/10_10/objs.pkl', 'wb') as objs_file:
    pickle.dump([init_test_x_and_y, train_x_set, train_y_set, model_save_path], objs_file)
