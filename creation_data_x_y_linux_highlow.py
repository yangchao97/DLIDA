...
"""
生成服从高斯分布（正态分布）的报价数据，
本code生成的报价为买家或者卖家虚高的报价，此报价直接影响payment和reward，
同时间接影响买卖双方的maxutility，
对于是否采用该报价计算SWMP需要进一步结合实际情况分析。
买卖双方对于计算资源的估值还是采用正常的报价。
2021/12/26
"""
...
def creation_data_x_y_high_low():
    mu_init_x = 2
    sigma_init_x = 1
    mu_init_y = 10
    sigma_init_y = 5
    ...
    sio.savemat('./mat_data/10_10/initial_x_high.mat', {'x': test_x_set[index * BATCH_SIZE].data.numpy()})
...
def test_creation_data_x_y():
    ...
    print("其中一个train_x_set样本的维度", train_x_set[150].size())
